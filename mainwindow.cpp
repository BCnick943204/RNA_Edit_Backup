...
QDialog* MainWindow::createAlgoList() {
    ...
    QString* names = gopher->getInfo(":/res/Config.xml", "name");
    QString* desc = gopher->getInfo(":/res/Config.xml", "desc");

    qInfo() << "Algo name: " << names;
    qInfo() << "Algo desc: " << desc;
...
void MainWindow::on_newSeqButton_clicked()
{
...
void MainWindow::on_uploadSeqButton_clicked()
{
    ...
    if(filename.contains(".txt")) {
        ...
        name.append(QString::number(FASTAcount));
        file_path = RNALIB::stringToFasta(sequence, directory_path, name);
        qInfo() << "File path for sequence file: " << file_path;
    } else {
        file_path = filename;
        qInfo() << "File path for sequence file: " << file_path;
...
void MainWindow::on_generateButton_clicked()
{
    ...
    ParseXML* xmlGopher = new ParseXML();

    qInfo() << "before loop";
    for (int i = 0; i < algoList->count(); i++) {
        if(algoList->item(i)->checkState() == Qt::Checked) {
            //Checks to see if we can load the algorithms.
            qInfo() << "got here, " << algoList->count();
            //                       loader(QString* libpath, QString name)
            IAlgorithm* algoObject = loader(xmlGopher->getPath(":/res/Config.xml", i), algoList->item(i)->text());
            if(algoObject == NULL) {
